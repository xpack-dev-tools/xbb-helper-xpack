...
function ncurses_build()
{
  ...
  local disable_widec="${XBB_NCURSES_DISABLE_WIDEC:-"n"}"
  local enable_lib_suffixes="y"
  ...
  then
    ...
    (
      ...
      then
        (
          ...
          echo
          echo "Running ncurses${suffix} configure..."
      ...
      (
        echo
        echo "Running ncurses${suffix} make..."
        ...
        then
          ...
          for lib in ncurses ncurses++ form panel menu tic tinfo
          do
    ...
    echo "Library ncurses${suffix} already installed"
  fi
}
