...
The easiest (and recommended) way to install {{ appName }} on { props.label } is by using the **binary xPack**, available as
<a href={ `https://www.npmjs.com/package/@xpack-dev-tools/{{ appLcName }}` }><code>@xpack-dev-tools/{{ appLcName }}</code></a>
...
* identify the platform specific archive for the latest available version
  of {{ appName }}, download it in a cache and unpack
...
it is also possible to install {{ appName }} only globally in the **central
