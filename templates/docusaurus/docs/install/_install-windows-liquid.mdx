...
`C:\> tree /f %USERPROFILE%\\AppData\\Roaming\\xPacks\\@xpack-dev-tools\\{{ appLcName }}\\${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }\\.content
Folder PATH listing
...
<CodeBlock language="console"> {
`C:\\> %USERPROFILE%\\AppData\\Roaming\\xPacks\\@xpack-dev-tools\\{{ appLcName }}\\${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }\\.content\\bin\\{{ appLcName }}.exe --version
...
<CodeBlock language="console"> {
`C:\\> %USERPROFILE%\\AppData\\Roaming\\xPacks\\{{ appLcName }}\\xpack-{{ appLcName }}-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }\\bin\\{{ appLcName }}.exe --version
${ customField('upstreamVersion') }`
} </CodeBlock>
...
As usual on Windows, mastering drivers is a challenge and {{ appName }} is no
exceptions, so don't be surprised to encounter many incompatible drivers
for various JTAG probes.

### Zadig

The {{ appName }} distribution includes some libusb
...
to WinUSB drivers will make {{ appName }} happy, please be aware that other
