...
- enable a filter on the current branch
- identify the commit ID from release notes or manifest.txt (search back in time)
- create a branch like <code>arm-{versionMajor()}-{{customFields.triplet}}-{versionMajor()}.{versionMinor()}.rel{customField('armSubRelease')}</code>
...
- cherry pick the 'system.h: do not define abort() on mingw'
- save as Patches
