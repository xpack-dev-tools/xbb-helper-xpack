...
- create a branch like <code>arm-{customField('gdbVersionMajor')}-{{customFields.triplet}}-{versionMajor()}.{versionMinor()}.rel{customField('armSubRelease')}</code>
...
the <code>arm-gnu-toolchain-{{customFields.triplet}}-abe-manifest.txt</code> file
