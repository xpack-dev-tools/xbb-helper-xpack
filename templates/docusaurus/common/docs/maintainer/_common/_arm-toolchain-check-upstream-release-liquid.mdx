...
<code>arm-gnu-toolchain-*-darwin-x86_64-{{customFields.triplet}}.tar.xz</code>)
and copy the file with the configurations
(<code>\*-darwin-x86_64-{{customFields.triplet}}-manifest.txt</code>) to extras.

Download the ABE manifest with the individual source URLs
(<code>arm-gnu-toolchain-{{customFields.triplet}}-abe-manifest.txt</code>) and copy to extras.
