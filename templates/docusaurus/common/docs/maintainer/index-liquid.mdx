...
title: How to make new xPack {{appName}} releases
...
```sh
rm -f ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git pull
...
```sh
xpm run check-space -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run clear-all-project-builds -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run generate-workflows -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
{% for platform in platforms_array %}
- **trigger-workflow-build-{{platform}}**{% endfor %}

```sh
{% for platform in platforms_array %}xpm run trigger-workflow-build-{{platform}} -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets{% endfor %}
```
...
{% if platform == "linux-arm64" or platform == "linux-arm" or platform == "win32-x64" %}{% assign isLinuxArm = true %}{% endif %}
{% endfor %}

To trigger the GitHub Actions tests, run the xPack actions:

- **trigger-workflow-test-prime**
{% if isLinuxX64 %}- **trigger-workflow-test-docker-linux-x64**{% endif %}
{% if isLinuxArm %}- **trigger-workflow-test-docker-linux-arm**{% endif %}

```sh
xpm run trigger-workflow-test-prime -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
{% if isLinuxX64 %}xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets{% endif %}
{% if isLinuxArm %}xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets{% endif %}
```
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-publish-release -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run update-package-binaries -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-test-xpm -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
npm dist-tag add @xpack-dev-tools/{{appLcName}}@${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')} latest
npm dist-tag ls @xpack-dev-tools/{{appLcName}}
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/{{appLcName}}@${customField('upstreamVersion')}-${customField('xpackSubversion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
in https://xpack-dev-tools.github.io/{{appLcName}}-xpack/blog/
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/{{appLcName}}-xpack.git/build-assets
```
...
[![Github All Releases](https://img.shields.io/github/downloads/xpack-dev-tools/{{appLcName}}-xpack/latest/total.svg)](https://github.com/xpack-dev-tools/{{appLcName}}-xpack/releases/)
  - all xPack releases [![Github All Releases](https://img.shields.io/github/downloads/xpack-dev-tools/{{appLcName}}-xpack/total.svg)](https://github.com/xpack-dev-tools/{{appLcName}}-xpack/releases/){% if customFields.showDeprecatedGnuMcuAnalytics == 'true' %}
  ...
  - [individual file counters](https://somsubhra.github.io/github-release-stats/?username=xpack-dev-tools&repository={{appLcName}}-xpack) (grouped by release)
