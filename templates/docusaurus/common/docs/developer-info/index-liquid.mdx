...
title: Information for xPack {{ appName }} developers
...
- <code>{{ branch }}</code>, with the current development version

All development is done in the <code>{{ branch }}</code> branch, and contributions via
Pull Requests should be directed to this branch.

When new releases are published, the <code>{{ branch }}</code> branch is merged
into `xpack`.
...
<details>
<summary>For development purposes, clone the <code>{{ branch }}</code> branch.</summary>
...
clone the <code>{{ branch }}</code> branch and link it to the central
xPacks store:
