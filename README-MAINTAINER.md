...
Determine the upstream version (like `2.1.23`) and eventually update the
`package.json` file; the format is `2.1.23-pre`.
...
and fix them; assign them to a milestone (like `2.1.23`).
...
Use the new version, suffixed by `pre`, like `2.1.23-pre`.

### Update `CHANGELOG.md`

- open the `CHANGELOG.md` file
- check if all previous fixed issues are in
- add a new entry like _* v2.1.23 released_
- commit with a message like _prepare v2.1.23_
...
- `npm dist-tag add @xpack-dev-tools/xbb-helper@2.1.23 latest`
- `npm dist-tag ls @xpack-dev-tools/xbb-helper`

If necessary, unpublish previous releases:

- `npm unpublish @xpack-dev-tools/xbb-helper@2.1.23`
