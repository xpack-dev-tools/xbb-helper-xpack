...
Determine the upstream version (like `1.10.3`) and eventually update the
`package.json` file; the format is `1.10.3-pre`.
...
and fix them; assign them to a milestone (like `1.10.3`).
...
- add a new entry like _* v1.10.3 released_
- commit with a message like _prepare v1.10.3_
...
- `npm dist-tag add @xpack-dev-tools/xbb-helper@1.10.3 latest`
- `npm dist-tag ls @xpack-dev-tools/xbb-helper`

If necessary, unpublish previous releases:

- `npm unpublish @xpack-dev-tools/xbb-helper@1.10.3`
