...
Determine the next version (like `4.3.12`) and update the
`package.json` file; the format is `4.3.12-pre`.
...
and fix them; assign them to a milestone (like `4.3.12`).
...
Use the new version, suffixed by `pre`, like `4.3.12-pre`.

### Update `CHANGELOG.md`

- open the `CHANGELOG.md` file
- check if all previous fixed issues are in
- add a new entry like _* v4.3.12 released_
- commit with a message like _prepare v4.3.12_
...
- https://github.com/xpack-dev-tools/xbb-helper-xpack/releases/tag/v4.3.12
