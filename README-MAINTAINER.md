...
Determine the next version (like `3.1.15`) and update the
`package.json` file; the format is `3.1.15-pre`.
...
and fix them; assign them to a milestone (like `3.1.15`).
...
Use the new version, suffixed by `pre`, like `3.1.15-pre`.

### Update `CHANGELOG.md`

- open the `CHANGELOG.md` file
- check if all previous fixed issues are in
- add a new entry like _* v3.1.15 released_
- commit with a message like _prepare v3.1.15_
...
- `npm dist-tag add @xpack-dev-tools/xbb-helper@3.1.15 latest`
- `npm dist-tag ls @xpack-dev-tools/xbb-helper`

If necessary, unpublish previous releases:

- `npm unpublish @xpack-dev-tools/xbb-helper@3.1.15`
