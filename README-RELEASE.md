...
Determine the upstream version (like `0.5.32`) and eventually update the
`package.json` file; the format is `0.5.32-pre`.
...
and fix them; assign them to a milestone (like `0.5.32`).
...
- add a new entry like _* v0.5.32 released_
- commit with a message like _prepare v0.5.32_
...
- `npm dist-tag add @xpack-dev-tools/xbb-helper@0.5.32 latest`
- `npm dist-tag ls @xpack-dev-tools/xbb-helper`

If necessary, unpublish previous releases:

- `npm unpublish @xpack-dev-tools/xbb-helper@0.5.32`
